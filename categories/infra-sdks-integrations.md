...
- [jev-handoff](https://github.com/shitianfang/jev-handoff) - MCP ecosystem: MCP server and library that batches typed Jev judgments for any MCP harness (plus a native Pi extension) and returns control to the LLM through typed escalations — untypeable and generation-needing questions are rejected before the call, low-confidence answers come back flagged as priors, and a fail-open PreToolUse gate can only deny or ask.
