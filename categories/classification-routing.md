...
- [Switchboard](https://github.com/ruban-24/switchboard) - Coding agents: open-source System One-powered router for Claude Code and Codex that asks Jev typed `Choice` questions to select the least sufficient model tier and model-specific reasoning effort, applies deterministic local policy, and pins the pair for each conversation to protect prompt-cache continuity; support for Laya, Kev, and Cua-S1 is coming soon.
